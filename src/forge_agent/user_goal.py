...
from .playbooks import PlaybookMatch, default_playbooks, explain_playbook_match
from .plugin_registry import default_plugin_registry
...
class UserGoalRunner:
    ...
    def run(self, goal: str, *, inputs: dict[str, Any] | None = None, mode: str = "preview") -> UserGoalResult:
        ...
        playbook_match = _best_playbook(goal)
        ...
        if skill is not None:
            ...
            if planned.missing_inputs:
                return UserGoalResult(goal, "input_required", _missing_text(planned.missing_inputs), normalized_mode, missing_inputs=planned.missing_inputs, playbook_match=playbook_match)
            if planned.plan is None:
                return UserGoalResult(goal, "no_plan", "I do not yet know how to do that safely.", normalized_mode, playbook_match=playbook_match)
            plan = planned.plan
            source = "planner"

        verdict = self.governance.evaluate_plan(plan)
        if verdict.decision == "block":
            return UserGoalResult(goal, "blocked", "I cannot continue with that plan.", normalized_mode, plan=plan, skill=skill, governance=verdict, playbook_match=playbook_match)
        if normalized_mode == "explain":
            return UserGoalResult(goal, "explained", explain_plan(plan, skill=skill, source=source, playbook_match=playbook_match), normalized_mode, plan=plan, skill=skill, governance=verdict, playbook_match=playbook_match)
        if normalized_mode == "preview" or verdict.decision == "confirm":
            ...
            text = "Please confirm before I continue." if verdict.decision == "confirm" else explain_plan(plan, skill=skill, source=source, playbook_match=playbook_match)
            return UserGoalResult(goal, status, text, normalized_mode, plan=plan, skill=skill, governance=verdict, playbook_match=playbook_match)
        ...
        return UserGoalResult(goal, status, text, normalized_mode, plan=plan, skill=skill, governance=verdict, execution=execution, promoted_skill=promoted, playbook_match=playbook_match)
...
def explain_plan(plan: TaskPlan, *, skill: SkillDefinition | None = None, source: str = "planner", playbook_match: PlaybookMatch | None = None) -> str:
    ...
    base = f"{intro} Source: {source}. I will do {len(plan.steps)} step(s): {steps}."
    if playbook_match is None:
        return base
    return f"{explain_playbook_match(playbook_match)} {base}"
...
def _best_playbook(goal: str) -> PlaybookMatch | None:
    matches = default_playbooks().match(goal, registry=default_plugin_registry(), limit=1)
